...
# 改进的权重计算方法 - 结合欧氏距离和相关性
def combined_weight(vector1, vector2):
    # 计算欧氏距离
    distance = euclidean(vector1, vector2)
    # 计算相关性
    correlation = np.corrcoef(vector1, vector2)[0, 1]
    # 组合权重
    adjusted_weight = ((correlation + 1) / 2) * np.exp(-distance)
    return adjusted_weight

# 构建网络并计算模块度的函数，增加了边缘权重阈值调整
def build_network_and_calculate_modularity(df_normalized, weight_function, edge_weight_threshold=0.0):
    G = nx.Graph()
    for i in range(n_domains):
        for j in range(i+1, n_domains):
            weight = weight_function(df_normalized.iloc[i], df_normalized.iloc[j])
            if weight > edge_weight_threshold:  # 调整边缘权重阈值
                G.add_edge(i, j, weight=weight)
...
# 执行实验并收集结果，尝试不同的边缘权重阈值
n_runs = 1
results = []

for run in range(1, n_runs + 1):
    mod_original = build_network_and_calculate_modularity(df_normalized, original_weight)
    mod_combined = build_network_and_calculate_modularity(df_normalized, combined_weight, edge_weight_threshold=0.1)
    results.append((run, mod_original, mod_combined))

# 将结果转换为DataFrame并打印
results_df = pd.DataFrame(results, columns=['Run', 'Modularity (Original)', 'Modularity (Combined)'])
print(results_df)
